...
const client = new Discord.Client({ disableMentions: 'everyone' , intents , partials: ['MESSAGE', 'CHANNEL', 'REACTION'] });
...
client.on('message', message => {
	 ...
	 if (message.content === 'alden') {
		message.reply('GAY');
	  }
	
	else if (message.content === 'test') {
		message.reply('Ada BAB1');
	}

	// else if (message.content === 'gay') {
	// 	message.reply('@Freshethos#6093');
	// }
...
client.on('message', message => {
	...
	switch(args[0]){
		case "ping":
		(cmd => cmd.name == args[0]);
		message.reply(`ping: ${client.ws.ping}ms:satellite_orbital:`)
		// .addField('ping', `${client.ws.ping}ms`)
		// message.channel.send(embed)
		break;

		case "info":
		message.reply('ga ada info ngab')
		break;
